...
4. VSA 경계, 파일 내부 책임 과밀, `ui` 내부 비즈니스 규칙 혼입, `shared` 승격 근거를 리뷰한다.
5. 필요 시 `next-best-practices`, `vercel-react-best-practices` 기준으로 Next.js/React 구조를 추가 검토한다.
6. 테스트, 타입체크, 린트, 빌드 등 필요한 검증 명령을 실제로 실행한다.
7. 명령 결과와 구조 리뷰 결과를 함께 판단해 최종 결론을 기록한다.
8. 결과를 `.agents/reports/validation/`에 한국어 Markdown 리포트로 저장한다.
