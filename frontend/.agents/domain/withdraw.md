...
- 실제 계정 삭제 API와 연결되어 탈퇴 성공 시 세션이 정리된 완료 흐름을 제공한다.
...
| `ConfirmWithdraw` | 사용자가 탈퇴하기 클릭 | 회원탈퇴 API 요청 후 완료 모달 오픈 |
...
| `WithdrawConfirmOpen` | `ConfirmWithdraw` | `WithdrawDoneOpen` | API 성공 시 쿠키를 정리하고 완료 모달 오픈 |
| `WithdrawDoneOpen` | `AcknowledgeWithdrawDone` | 외부 홈 상태 | `/`로 이동 |

## Invariants
- `Withdraw`는 탈퇴 사유 선택, 모달 상태 전이, 탈퇴 요청 orchestration을 auth feature hook이 소유하고, `WithdrawReasonForm`은 렌더링과 접근성 연결만 담당한다.
...
| `useWithdrawReasonForm` | UIArtifact | `src/features/auth/hooks/useWithdrawReasonForm.ts` |
...
| `WithdrawReasonForm` 상태/전이 | `src/features/auth/hooks/useWithdrawReasonForm.ts` |
| `Withdraw` 정적 카피 | `src/shared/static/auth` |
| `Withdraw` BFF route | `src/app/api/auth/withdraw/route.ts` |
...
| `WithdrawReasonForm` | `uses` | `useWithdrawReasonForm` |
...
- `WithdrawReasonForm`은 state 없이 렌더링과 접근성 연결만 담당해야 한다.
- 사유 선택, 모달 open state, 탈퇴 성공/실패 전이는 `useWithdrawReasonForm`이 소유해야 한다.
