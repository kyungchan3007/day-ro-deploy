...
import { DELETE as withdrawRoute } from "../../../app/api/auth/withdraw/route";
...
describe("Server auth", () => {
  ...
  it("withdraw route returns 401 when no access token cookie exists", async () => {
    const request = new NextRequest("http://localhost:3000/api/auth/withdraw", {
      method: "DELETE",
    });

    const response = await withdrawRoute(request);
    const body = await response.json();
  ...
  it("withdraw route clears local cookies after backend withdraw succeeds", async () => {
    vi.mocked(fetch).mockResolvedValue(
      new Response(JSON.stringify(withdrawSuccessPayload), {
        status: 200,
        headers: {
          "Content-Type": "application/json",
        },
    ...
    expect(fetch).toHaveBeenCalledWith(
      new URL("/api/auth/withdraw", "http://localhost:8080"),
      expect.objectContaining({
        method: "DELETE",
        headers: {
    ...
    expect(response.cookies.get(ACCESS_TOKEN_COOKIE_NAME)?.value).toBe("");
    expect(response.cookies.get(REFRESH_TOKEN_COOKIE_NAME)?.value).toBe("");
    expect(body).toEqual(withdrawSuccessPayload);
  });

  it("withdraw route keeps local cookies when backend withdraw fails", async () => {
    vi.mocked(fetch).mockRejectedValue(new Error("network_error"));
    ...
    expect(response.cookies.get(ACCESS_TOKEN_COOKIE_NAME)?.value).toBeUndefined();
    expect(response.cookies.get(REFRESH_TOKEN_COOKIE_NAME)?.value).toBeUndefined();
    expect(body).toEqual({
      success: false,
