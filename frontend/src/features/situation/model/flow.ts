...
export const SITUATION_STEPS = [
  ...
  { key: "region", nextLabel: "다음은 어떤 만남인가요?" },
  { key: "purpose", nextLabel: "추천받기!" },
] as const;

export type SituationStepKey = (typeof SITUATION_STEPS)[number]["key"];
/** 위저드 스텝(time/region/purpose) + 후속 화면(loading/result/course). */
export type SituationFlowStep =
  | SituationStepKey
  | "loading"
  | "result"
  | "course";
...
/** 선택 완료 후 확정 코스(지도 + 경로) 화면. 진행바 카운트에는 포함하지 않는다. */
export const SITUATION_COURSE_STEP: SituationFlowStep = "course";
...
export function resolveSituationStep(
  ...
  if (rawStep === SITUATION_COURSE_STEP) {
    return SITUATION_COURSE_STEP;
  }
