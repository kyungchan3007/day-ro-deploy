 ...
 *   - 데이트 길이는 최소 2시간 ~ 최대 12시간일 때만 유효하다.
 */
export function useTimeRangeStep(initial: TimeRange) {
  ...
  const belowMinLimit = belowMinDuration(range);
  const isValid = !belowMinLimit && !exceedsLimit;
  const overnight = isOvernight(range);
  const durationLabel = formatDuration(duration);
  const minDurationLabel = formatDuration(MIN_SITUATION_DURATION_MINUTES);
