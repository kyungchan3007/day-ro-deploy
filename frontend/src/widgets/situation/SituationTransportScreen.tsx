...
import { AppShell, Button, Container, StepProgress } from "@/shared/ui";
...
import styles from "./css/SituationTransportScreen.module.css";
...
const Eyebrow = ({ children }: { children: React.ReactNode }) => (
  <span className={`${styles.eyebrow} text-xs font-bold text-primary`}>
    {children}
  </span>
);
    ...
    >
      ...
      <Container className={styles.content}>
        ...
        <section>
          ...
          <p className={`${styles.groupCaption} text-sm text-text-muted`}>
            지금 위치에서 {destinationLabel}까지
          </p>
          <h2 className={`${styles.groupTitle} text-xl font-bold text-text-strong`}>
            어떻게 갈까요?
          </h2>
          <TransportCardGroup className={styles.cardGroup} value={go} onChange={setGo} />
        </section>
